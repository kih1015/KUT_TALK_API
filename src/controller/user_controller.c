...
/* ---------- 공통 헬퍼 ---------- */
static int build_json_resp(char *buf, size_t sz,
                           enum http_status st,
...
static const char *find_cookie(const struct http_request *req,
                               const char *key)
{
...
/* ---------- 회원가입 ---------- */
int user_controller_register(const struct http_request *req,
                             char *resp_buf, size_t resp_sz)
{
    ...
    switch (rc) {
        ...
        case -1:
            return build_json_resp(resp_buf, resp_sz, HTTP_CONFLICT,
                                   "{\"error\":\"Duplicate userid/nickname\"}");
        default:
            return build_json_resp(resp_buf, resp_sz, HTTP_INTERNAL_ERROR,
                          ...
                          char *resp_buf, size_t resp_sz)
{
    ...
    if (rc == 0) {   /* 성공 */
        int n = snprintf(resp_buf, resp_sz,
            "HTTP/1.1 200 OK\r\n"
            "Content-Type: application/json\r\n"
...
/* ---------- 내 정보 ---------- */
int user_controller_get_me(const struct http_request *req,
                           char *resp_buf, size_t resp_sz)
{
...
/* ---------- 로그아웃 ---------- */
int user_controller_logout(const struct http_request *req,
                           char *resp_buf, size_t resp_sz)
{
    const char *sid = find_cookie(req, "KTA_SESSION_ID");
    if (!sid)
